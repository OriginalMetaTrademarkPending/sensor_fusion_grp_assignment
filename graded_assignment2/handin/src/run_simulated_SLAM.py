...
def main():
    ...
    # %% Initilize
    # Q = np.diag([100*0.1, 100*0.1, 10*np.pi/180])**2
    # R = np.diag([10*0.1, 10*np.pi/180])**2
    Q = np.diag([100*0.1, 100*0.1, 10 * np.pi / 180]) ** 2 # TODO tune
    R = np.diag([10*0.1, 10 * np.pi / 180]) ** 2 # TODO tune
    ...
    # Some code for generating a .gif file.
    from matplotlib.animation import FuncAnimation

    # Set up the figure and axis
    fig_movie, ax_movie = plt.subplots(num=7, clear=True)
    ax_movie.grid()
    ax_movie.set(xlim=(mins[0], maxs[0]), ylim=(mins[1], maxs[1]))
    ...
    def update(frame):
        ...
        ax_movie.scatter(*landmarks.T, c="r", marker="^")
        ax_movie.plot(*poseGT[:frame, :2].T, "r-")
        ax_movie.plot(*pose_est[:frame, :2].T, "g-")
        ax_movie.scatter(*lmk_est[frame].T, c="b", marker=".")

        if frame > 0:
            el = ellipse(pose_est[frame, :2], P_hat[frame][:2, :2], 5, 200)
            ax_movie.plot(*el.T, "g")

        numLmk = lmk_est[frame].shape[0]
        for l, lmk_l in enumerate(lmk_est[frame]):
            idxs = slice(3 + 2 * l, 3 + 2 * l + 2)
            rI = P_hat[frame][idxs, idxs]
            el = ellipse(lmk_l, rI, 5, 200)
            ax_movie.plot(*el.T, "b")

    # Create the animation
    animation = FuncAnimation(fig_movie, update, frames=range(N), blit=False)

    # Save the animation frames as a GIF with a transparent background using Pillow
    animation.save('bodegaSLAM.gif', writer='pillow')
